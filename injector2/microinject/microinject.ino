...
#define ABS_MAX_SPEED 600.0f
#define ABS_MAX_ACCEL 400.0f
...
struct Step {
  ...
  float
      end_speed; // steps/s at end of move (clamped; distance takes precedence)
...
void printProgram() {
  ...
  for (uint8_t i = 0; i < programCount; i++) {
    ...
    Serial.print(F("         "));
    Serial.print((int)program[i].start_speed);
    Serial.print(F("        "));
    Serial.print((int)program[i].end_speed);
    Serial.print(F("     "));
...
Step promptStep() {
  ...
  st.start_speed =
      promptFloat("  Start spd (steps/s, 1-600) > ", 1.0f, ABS_MAX_SPEED);
  st.end_speed =
      promptFloat("  End spd   (steps/s, 1-600) > ", 1.0f, ABS_MAX_SPEED);
  st.accel = promptFloat("  Accel     (steps/s2, -400 to 400) > ",
                         -ABS_MAX_ACCEL, ABS_MAX_ACCEL);
  return st;
}
...
float computeSpeed(long steps_done) {
  if (g_accel == 0.0f || g_start_speed == g_end_speed)
    return g_start_speed;

  float v2 = g_start_speed * g_start_speed + 2.0f * g_accel * (float)steps_done;
  if (v2 < 0.0f)
    v2 = 0.0f; // deceleration floored at zero
  float v = sqrtf(v2);
...
void loop() {
  ...
  case 'R': {
    if (programCount == 0) {
      Serial.println(F("! Program empty."));
      printMainMenu();
      break;
    }
    progStepIndex = 0;
